...
Upon receiving a Convertible Upgrade Request, an HTTP intermediary MAY convert it into an Extended CONNECT request (of any HTTP version) using ordinary HTTP version translation with the following modifications:
...
If the intermediary receives any other response, it MUST NOT convert it to an HTTP/1.1 Upgrade response, and SHOULD forward it using ordinary HTTP version translation.  If the response was not 1xx (Informational), the intermediary MAY accept additional HTTP/1.1 requests on this connection to the client.
...
If the intermediary receives a "101 (Switching Protocols)" response, it MUST change the response code to "200 (OK)".  Otherwise, it MUST forward any responses unmodified.  After sending this response, the intermediary MUST process all data to and from the server in accordance with the Capsule Protocol.

## Converting an Extended CONNECT request to Extended CONNECT for a different HTTP version

An HTTP intermediary MAY convert Convertible Extended CONNECT Requests between different HTTP versions using  ordinary HTTP version translation.
...
An HTTP intermediary implementing this specification MUST account for backend servers that support HTTP/2 or HTTP/3, but do not support Extended CONNECT.  When the backend server does not support Extended CONNECT, the intermediary MUST transmit the request over HTTP/1.1.
...
* Intermediaries' behavior regarding each capsule type is independent of the CPUT.  CPUTs cannot change the intermediary treatment of existing capsule types.
